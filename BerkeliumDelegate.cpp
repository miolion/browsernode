...
if (!m_pBuffer) {
    ...
    if (dx != 0 || dy != 0) {
        ...
        if (scrolled_shared_rect.width() > 0 && scrolled_shared_rect.height() > 0) {
            ...
            int source = (yInit - dy)*m_Size.x + (left - dx);
            int dest = yInit*m_Size.x + left;
            size = (right - left)*BYTES_PER_PIXEL;
            for (int y = yInit; y >= top && y < bottom; y += yInc) {
                memmove(&pBuffer[dest], &pBuffer[source], size);
                source += increment;
                dest += increment;
            }
            if (m_pPrintBuffer != m_pBuffer ) {
                int start = (m_Offset.y > top) ? m_Offset.y : top; 
                if (left > m_Offset.x) {
                    source = (start)*m_Size.x + left;
                    dest = (left - m_Offset.x) + (start - m_Offset.y)*m_PrintSize.x;
                    size = (right - left)*BYTES_PER_PIXEL;
                } else {
                    source = (start)*m_Size.x + m_Offset.x;
                    dest = (start - m_Offset.y)*m_PrintSize.x;
                    size = (right - m_Offset.x)*BYTES_PER_PIXEL;
                }
                for (int y = start; y < bottom; y++) {
                    memcpy(&pPrintBuffer[dest], &pBuffer[source], size);
                    source +=  m_Size.x;
                    dest += m_PrintSize.x;
                }
            }
    ...
    for (size_t i = 0; i < numCopyRects; i++) {
        ...
        int sourceLine = (top-bitmapRect.top())*bitmapRect.width() + (left-bitmapRect.left());
        int destLine = left + top*m_Size.x;
        for (int y = top; y < bottom; ++y) {
            int source = sourceLine*BYTES_PER_PIXEL;
            int dest = destLine*BYTES_PER_PIXEL;
            for (int x = left; x < right; ++x) {
                m_pBuffer[dest+2] = pBitmapIn[source];   // B
                m_pBuffer[dest+1] = pBitmapIn[source+1]; // G
                m_pBuffer[dest] = pBitmapIn[source+2];   // R
                m_pBuffer[dest+3] = pBitmapIn[source+3]; // A
                source += BYTES_PER_PIXEL;
                dest += BYTES_PER_PIXEL;
            }
            sourceLine += bitmapRect.width();
            destLine += m_Size.x;
        }
        if (m_pPrintBuffer != m_pBuffer) {
            uint32_t *pBuffer = (uint32_t*)m_pBuffer;
            uint32_t *pPrintBuffer = (uint32_t*)m_pPrintBuffer;
            int yInit = (m_Offset.y > top) ? m_Offset.y : top ;
            int xInit = (m_Offset.x > left) ? m_Offset.x : left ;
            int source = xInit + yInit*m_Size.x;
            int dest = (xInit - m_Offset.x)  + (yInit - m_Offset.y)*m_PrintSize.x;
            int size = (right - xInit)*BYTES_PER_PIXEL;
            for (int y = yInit; y < bottom; ++y) {
                memcpy(&pPrintBuffer[dest], &pBuffer[source], size);
                source += m_Size.x;
                dest += m_PrintSize.x;
            }
...
void onCreatedWindow(Window *win, Window *newWindow,
